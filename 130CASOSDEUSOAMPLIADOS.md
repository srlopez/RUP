...
Caso de uso 02: Orden de pago 
    ...
    3. El sistema presenta los costes de envío, la fecha de llegada programada y una lista de tarjetas de crédito ya registradas para el cliente (marca y los últimos cuatro dígitos del número de la tarjeta de crédito). 
...
(se une con Pedir libros y Pagar pedido)
    1. El cliente proporciona palabras clave para la búsqueda de libros. 
    2. El sistema genera una lista de libros a la venta que coincide con las palabras clave, incluyendo al menos el título, autor, precio, número de páginas, editorial, ISBN e imagen de portada. 
    3. El cliente selecciona los libros de la lista e indica la cantidad deseada para cada uno. 
    4. El sistema genera el resumen de la orden (título, autor, cantidad, precio unitario y subtotal para cada libro) y el valor total. 
...
Algunos analistas pueden elegir una de las variantes para que esté en el flujo principal y la otra para que sea un flujo alternativo en forma de variante. En la Figura 5.5 se eligió la variante Proceder al pago para que estuviera en el flujo principal, mientras que la variante Guardar carro de la compra se dejó fuera del flujo principal. Una ventaja de esta elección es que permite asociar la variante a más de un paso en el flujo principal. Por ejemplo, tenga en cuenta que la variante Grabar cesta de la compra puede ejecutarse cuando el usuario se encuentra en cualquier paso del 1 al 8. En este caso el código de la variante sería el 1-8a. Si la variante pudiera ejecutarse en cualquier momento durante el flujo principal, su número sería *a.
 ...
 Caso de uso 01/02: Comprar libros 
    ...
    5. El cliente finaliza el pedido.
    6. El sistema genera una lista de direcciones registradas para el cliente.
    7. El cliente selecciona una dirección de entrega.
    8. El sistema presenta la tarifa de entrega, la fecha de llegada programada y una lista de tarjetas de crédito ya registradas para el cliente.
    9. El cliente selecciona una de sus tarjetas para el pago. 
    10. El sistema envía al operador de la tarjeta de crédito correspondiente los siguientes datos: número de tarjeta, nombre del propietario, validez, código de seguridad, valor total de la compra y código de la tienda. 
    11. El operador de la tarjeta de crédito aprueba la venta enviando un código de autorización.
    12. El sistema informa al cliente de que la venta ha sido autorizada y le proporciona el número de seguimiento del paquete. 
...
*Contrariamente a lo que se dijo en el Capítulo 2, aquí asumimos por simplicidad que los libros agotados no pueden ser vendidos.
...
(falta información obligatoria) EVITAR! 
    1. El cliente proporciona palabras clave para la búsqueda de libros.
    2. El sistema presenta una lista de libros a la venta que coincide con las palabras clave, incluyendo al menos el título, autor, precio, número de páginas, editorial, ISBN e imagen de portada. 
    3. El sistema genera el resumen del pedido (título. autor, cantidad, precio unitario y subtotal para cada libro) y valor total. 
    4. El cliente finaliza el pedido. 
...
(con pasos inadecuados) EVITAR! 
    1. El cliente proporciona palabras clave para la búsqueda de libros.
    2. [X] El sistema verifica en la base de datos los libros que cumplen los criterios de búsqueda.
    3. El sistema genera una lista de libros a la venta que coincide con las palabras clave, incluyendo al menos el título, autor, precio, número de páginas, editor. ISBN. e imagen de portada.
    4. El cliente selecciona los libros de la lista e indica la cantidad deseada para cada uno.
    5. [X] El sistema calcula el total de la venta como la suma de los subtotales, que consisten en el precio de cada libro multiplicado por la cantidad deseada respectiva. 
    6. El sistema genera el resumen de la orden (título, autor, cantidad, precio unitario y subtotal para cada libro) y el valor total.
    7. El cliente finaliza el pedido.
    8. [X] El sistema marca la orden como "terminada" y la almacena en la base de datos. 
...
Caso de uso 02: Orden de pago
    ...
    2. El cliente selecciona una dirección de entrega.
    3. El sistema presenta los costes de envío y la fecha de llegada programada,
    4. Incluya el Fragmento 02a: Pago.
    5. El sistema informa al cliente de que la venta ha sido autorizada y le proporciona el número de seguimiento del paquete.
...
Caso de Uso plantilla: <<report>> Informe sobre .... 
    ...
    2. El sistema muestra … agrupadas por … y ordenadas por ...
...
( I a.1,1c.2)a.2: Volver al paso que causó la excepción. 
Excepción 1d.1a: Hay libros asociados al editor. 
1d.la. I: El sistema informa al usuario de que no es posible eliminar un editor con libros asociados. 
1d.la.2: Volver al paso ld.l. 
...
Las actividades relacionadas con el análisis de casos de uso todavía no tratan con objetos que son internos al sistema. Por lo tanto, el sistema debe representarse como un único objeto: una caja negra. En este caso, puede representarse con el símbolo de interfaz, tal y como proponen Jacobson y otros (1992). Un actor sólo puede comunicarse con un sistema a través de su interfaz. Los mensajes del actor no pueden llegar a los objetos, como se muestra en la Figura 5.23.
...
Hay que hacer una observación sobre el comando createShoppingCart: a medida que crea un nuevo objeto de dominio y devuelve su código de identificación, uno podría preguntarse si esto no va en contra del principio de separación comando-consulta. No lo es. Es una de las excepciones aceptables a ese principio, porque el comando sólo crea un nuevo objeto y devuelve su código de identificación. Puede hacerse porque intentar obtener el código de un objeto recién creado por otros medios sería contraproducente.
...
Observe que en la Figura 5.38 el fragmento que maneja la excepción simplemente desaparece. Esto sucedió porque la excepción se convirtió en una condición previa para el comando de sistema add2Cart. Esta es una técnica popular para mejorar el diseño que se revisará en el Capítulo 8. La técnica consiste en evitar que el usuario envíe un argumento inválido; la elección del usuario se limita a los valores válidos. Por lo tanto, la excepción no puede volver a ocurrir.
...
2Por ejemplo, un cliente compra dos libros y otro cliente compra cinco libros: ambas acciones pueden describirse simplemente como "el cliente compra libros" porque el número de libros es irrelevante para entender el proceso.
3Sin embargo, este estilo para las variantes no debe animar al lector a hacer lo mismo con excepciones. Como se explica en la Sección 5.3.3, en el caso de las excepciones hay un flujo normal y otro alternativo, que deben describirse fuera del flujo principal; en el caso de las variantes, la idea es que sólo son maneras diferentes de realizar el flujo principal.
4El conjunto {1, 2, 3, ...}.
5Aquí y en otros ejemplos de este libro, el ID proporcionado por un cliente no es un código interno generado por el sistema, sino el número de un documento personal que pertenece al cliente. En los Estados Unidos el número de seguro social podría corresponder al número de identificación, mientras que en otros países podrían utilizarse otros números.
6 En el pasado el autor ejecutó un proyecto en el que un caso de uso que parecía ser un simple CRUD al principio resultó ser el caso de uso más complejo de todo el sistema al final.
7Un controlador-fachada proporciona "una interfaz unificada a un conjunto de interfaces en un subsistema". Fachada define una interfaz de nivel superior que hace que el subsistema sea más fácil de usar" (Gamma, Helm, Johnson y Vlissides, 1995).
8Recuerde que no siempre es el caso que la información compleja intercambiada con el sistema corresponde al conjunto completo de atributos de un concepto del modelo conceptual.
9La estrategia aplicada en la Figura 5.40 es de estado. Si en su lugar se utilizara la estrategia de los sin estado, las operaciones serían ligeramente diferentes, porque tendrían más parámetros..
